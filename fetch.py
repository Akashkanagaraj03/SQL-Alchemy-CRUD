...
#fetching reviews of a certain movie
...
#fetching all movies reviews by a certain user
...
statement_2 = session.query(Review.movie_id).join(User).filter(User.id == 4)   #uses orm
result_2 = session.query(Movie).filter(Movie.id.in_(statement_2)).all()
